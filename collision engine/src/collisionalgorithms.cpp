...
void particleCollisionEquation(Particle* p, Particle* q, double e) {
  ...
  double u1xr = (v1r.x * (m1 - e * m2) + (1.0 + e) * m2 * v2r.x) / (m1 + m2);
  double u2xr = (v2r.x * (m2 - e * m1) + (1.0 + e) * m1 * v1r.x) / (m1 + m2);
...
void particleRigidCollisionEquation(Particle* p, Particle* q, double e) {
  ...
  double u1xr = (- v1r.x * e + (1.0 + e) * v2r.x);
...
double calculateNValue(Particle* p, Particle* q, double dt) {
  ...
  double c = d.dot(d) - pow(p->radius + q->radius, 2);

  double disc = (b * b) - (4.0 * a * c);

  if (a == 0.0 || disc < 0.0) return 0.0;
